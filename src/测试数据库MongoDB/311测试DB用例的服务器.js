...
import('./311ConnectDB.js');    //执行js文件代码,连接数据库
import { User } from './311CreateUserTable.js'
//  const User = import('./311CreateUserTable.js');   //创建并连接数据表对象
...
app.on('request', async (req, res) => {
	...
	// 请求地址，query本来是字符串，现在转换为对象，用于获取请求参数。
	const { pathname, query } = url.parse(req.url, true);
	console.log('接收到的请求url：' + pathname)
	if (method == 'GET') {
		// 呈现用户列表页面
		if (pathname == '/list') {
			// 查询用户信息
			...
			// res.end(query.id)
			//修改并删除数据库中的一条数据，await关键字必须和asyn关键字同时使用
